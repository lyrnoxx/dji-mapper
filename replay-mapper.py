...
class MultiBandMap2D:
    def __init__(self, resolution=1.5, band_num=2, tile_size=512):
        ...
        self.paused = False
        self.history = []  # Stores (frame, pose_matrix)

    def reset(self):
        """Clears the generated map tiles."""
        with self.lock:
            self.tiles = {}
    ...
    def feed(self, frame, pose_matrix, camera_matrix, plane_height=0.0, store_history=True):
        if self.paused and store_history: return False
        
        # Save to history for replay/undo
        if store_history:
            self.history.append((frame.copy(), pose_matrix.copy()))
            # Limit history to 500 frames to save memory
...
if __name__=="__main__":
    ...
    print("Controls: [P] Pause | [S] Save | [R] Replay All | [B] Go Back (Undo) | [ESC] Exit")
    
    threading.Thread(target=lambda: handle_conn(s.accept()[0], mapper, K), daemon=True).start()
    ...
    while True:
        ...
        if img is not None:
            ...
            if mapper.paused:
                cv2.putText(display_img, "PAUSED", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        ...
        # [S] Save Snapshot
            ...
            if img is not None:
                cv2.imwrite(f"map_{int(time.time())}.png", img)
        ...
        # [R] Replay All
            ...
            mapper.reset()
            for f, p in mapper.history:
                mapper.feed(f, p, K, store_history=False)

        # [B] Go Back (Step Back / Undo)
        elif key == ord('b') or key == ord('B'):
            if len(mapper.history) > 5:
                print("Stepping back...")
                mapper.history = mapper.history[:-5] # Remove last 5 frames
                mapper.reset()
                for f, p in mapper.history:
                    mapper.feed(f, p, K, store_history=False)
            else:
                print("History too short to step back.")
