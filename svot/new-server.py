...
class MultiBandMap2D:
    def __init__(self, resolution=0.05, band_num=2, tile_size=512):
        ...
        # KEYFRAME LOGIC VARIABLES
        self.last_key_pose = None
        self.dist_threshold = 70.0  # Meters (Accept frame every 5m of travel)
        self.rot_threshold = 5.0   # Degrees (Accept frame every 5° of turn)
    ...
    def feed(self, frame, pose_matrix, camera_matrix, plane_height=0.0):
        ...
        if self.last_key_pose is not None:
            prev_pos = self.last_key_pose[:3, 3]
            prev_rot = self.last_key_pose[:3, :3]

            # Calculate Euclidean distance
            dist_moved = np.linalg.norm(curr_pos - prev_pos)

            # Calculate rotation change (Angular distance)
            # Trace of (R1^T * R2) = 1 + 2*cos(theta)
            R_diff = np.dot(prev_rot.T, curr_rot)
            trace = np.trace(R_diff)
            # Clip to avoid math errors
            angle_changed = np.degrees(np.arccos(np.clip((trace - 1) / 2, -1, 1)))

            # If movement is below BOTH thresholds, skip this frame
            if dist_moved < self.dist_threshold and angle_changed < self.rot_threshold:
                return False 

        # 3. If we passed the check, save this as the new last_key_pose
        self.last_key_pose = pose_matrix.copy()
